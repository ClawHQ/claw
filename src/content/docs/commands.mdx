...
Create `.ishi/command/test.md`:

```md title=".ishi/command/test.md"
---
...
You can add custom commands through the Ishi config or by creating markdown files in the `command/` directory.

---

### JSON

Use the `command` option in your Ishi [config](/docs/config):

```json title="ishi.jsonc" {4-12}
{
...
- Global: `~/.config/ishi/command/`
- Per-project: `.ishi/command/`

```markdown title="~/.config/ishi/command/test.md"
---
description: Run tests with coverage
agent: build
model: anthropic/claude-3-5-sonnet-20241022
...
```md title=".ishi/command/component.md"
---
...
```md title=".ishi/command/create-file.md"
---
...
```md title=".ishi/command/analyze-coverage.md"
---
...
```md title=".ishi/command/review-changes.md"
---
...
```md title=".ishi/command/review-component.md"
---
...
ishi includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
