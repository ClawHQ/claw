---
title: Custom Tools
description: Create tools the LLM can call in ishi.
---

Custom tools are functions you create that the LLM can call during conversations. They work alongside ishi's [built-in tools](/docs/tools) like `read`, `write`, and `bash`.
...
- Locally by placing them in the `.ishi/tool/` directory of your project.
- Or globally, by placing them in `~/.config/ishi/tool/`.
...
```ts title=".ishi/tool/database.ts" {1}
import { tool } from "@ishilabs/plugin"
...
```ts title=".ishi/tool/project.ts" {8}
import { tool } from "@ishilabs/plugin"
...
```ts title=".ishi/tool/python-add.ts" {10}
import { tool } from "@ishilabs/plugin"
...
export default tool({
  ...
  async execute(args) {
    const result = await Bun.$`python3 .ishi/tool/add.py ${args.a} ${args.b}`.text()
    return result.trim()
  },
})
```
