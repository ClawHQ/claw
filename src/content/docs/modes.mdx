...
:::caution
Modes are now configured through the `agent` option in the ishi config. The
`mode` option is now deprecated. [Learn more](/docs/agents).
:::

Modes in ishi allow you to customize the behavior, tools, and prompts for different use cases.

It comes with two built-in modes: **build** and **plan**. You can customize
these or configure your own through the ishi config.
...
claw comes with two built-in modes.
...
Configure modes in your `ishi.json` config file:
...
- Global: `~/.config/ishi/mode/`
- Project: `.ishi/mode/`

```markdown title="~/.config/ishi/mode/review.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.1
tools:
...
both the global ishi config and the project specific config.
...
Create mode files in `.ishi/mode/` for project-specific modes or `~/.config/ishi/mode/` for global modes:

```markdown title=".ishi/mode/debug.md"
---
temperature: 0.1
tools:
  bash: true
...
```markdown title="~/.config/ishi/mode/refactor.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.2
tools:
