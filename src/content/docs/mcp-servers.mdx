...
You can add external tools to Ishi using the _Model Context Protocol_, or MCP.
...
You can define MCP servers in your Ishi config under `mcp`. Add each MCP
with a unique name. You can refer to that MCP by name when prompting the LLM.

```jsonc title="ishi.jsonc" {6}
{
...
Ishi automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Ishi will:
...
If the server requires authentication, Ishi will prompt you to authenticate when you first try to use it.
...
ishi mcp auth my-oauth-server

# List all MCP servers and their auth status
ishi mcp list

# Remove stored credentials
ishi mcp logout my-oauth-server
```

The `mcp auth` command will open your browser for authorization. After you authorize, Ishi will store the tokens securely in `~/.local/share/ishi/mcp-auth.json`.

---

## Manage

Your MCPs are available as tools in Ishi, alongside built-in tools. So you
can manage them through the Ishi config like any other tool.
