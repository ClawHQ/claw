...
The `ishi serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an ishi client can use.
...
```bash
ishi serve [--port <number>] [--hostname <string>]
```
...
When you run `ishi` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the ishi server to interact with ishi programmatically.
:::

This architecture lets ishi support multiple clients and allows you to interact with ishi programmatically.

You can run `ishi serve` to start a standalone server. If you have the
claw TUI running, `ishi serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Ishi [IDE](/docs/ide) plugins.
...
The ishi server exposes the following APIs.
