...
With Ishi you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Ishi uses our own `ishi` theme.
...
Ishi comes with several built-in themes.
...
- Want Ishi to match their terminal's appearance
...
Ishi supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/ishi/themes/*.json` or `$XDG_CONFIG_HOME/ishi/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.ishi/themes/*.json`
4. **Current working directory** - Defined in `./.ishi/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/ishi/themes
vim ~/.config/ishi/themes/my-theme.json
```
...
{
  "$schema": "https://ishi.so/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
