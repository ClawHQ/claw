---
title: Providers
description: Using any LLM provider in Ishi.
---

import config from "../../../config.mjs"
export const console = config.console

Ishi uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support for **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your Ishi config.
...
in `~/.local/share/ishi/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your Ishi
config.
...
Pixel ML OpenRouter is a unified gateway to access all major AI models through a single API. Our flagship **Ishi Intent Engine** intelligently routes requests to save you money. [Learn more](/docs/pixelml).

:::tip
If you are new, we recommend starting with Pixel ML OpenRouter. Every Ishi customer gets **$1 free credit**.
:::

1. Run the `/connect` command in the TUI, select Pixel ML, and head to [ishi.so/auth](https://ishi.so/auth).

   ```txt
   /connect
   ```

2. Sign in—your $1 free credit is automatically applied. Add billing details for continued use at [platform.pixelml.com](https://platform.pixelml.com/).
...
4. Run `/models` in the TUI—we recommend **semantic-routed-model** for intelligent cost-saving routing.

   ```txt
   /models
   ```

The API endpoint is `https://ishi.pixelml.com/v1/chat/completions` (OpenAI-compatible).
...
To use Amazon Bedrock with Ishi:
...
1. You'll need either to set one of the following environment variables:
   ...
   Once you have one of the above, set it while running ishi.

   ```bash
   AWS_ACCESS_KEY_ID=XXX claw
   ```
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for ishi to work properly.
   :::
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX claw
   ```
...
To use your GitHub Copilot subscription with ishi:
...
To use Google Vertex AI with Ishi:
...
2. Set the required environment variables:
   ...
   Set them while running ishi.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id claw
   ```
...
In the event you see a feature or model from Helicone that isn't configured automatically through ishi, you can always configure it yourself.
...
```jsonc title="~/.config/ishi/ishi.jsonc"
{
  "$schema": "https://ishi.so/config.json",
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [ishi-helicone-session](https://github.com/H2Shami/ishi-helicone-session) plugin to automatically log each Ishi conversation as a session in Helicone.

```bash
npm install -g ishi-helicone-session
```
...
{
  "plugin": ["ishi-helicone-session"]
}
```

The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each Ishi conversation listed as a separate session.
...
You can configure ishi to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility

```json title="ishi.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://ishi.so/config.json",
  "provider": {
    "llama.cpp": {
...
You can configure ishi to use local models through LM Studio.

```json title="ishi.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://ishi.so/config.json",
  "provider": {
    "lmstudio": {
...
You can configure ishi to use local models through Ollama.

```json title="ishi.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://ishi.so/config.json",
  "provider": {
    "ollama": {
...
3. Copy the API key for use in Ishi.
...
6. **Important**: Before using cloud models in Ishi, you must pull the model information locally:
...
Pixel ML OpenRouter is a list of tested and verified models provided by the Ishi team. [Learn more](/docs/pixelml).

1. Sign in to **<a href={console}>Pixel ML OpenRouter</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **Pixel ML OpenRouter**.

   ```txt
   /connect
   ```

3. Enter your Ishi API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your ishi config.
...
5. You can also customize them through your ishi config. Here's an example of specifying a provider
...
3. Enter your service key JSON.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' claw
   ```
...
4. Optionally set deployment ID and resource group:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group claw
   ```
...
:::tip
You can use any OpenAI-compatible provider with ishi. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in ishi.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `ishi.json` file in your project directory:

   ```json title="ishi.json" ""myprovider"" {5-15}
   {
     "$schema": "https://ishi.so/config.json",
     "provider": {
       "myprovider": {
...
```json title="ishi.json" {9,11,17-20}
{
  "$schema": "https://ishi.so/config.json",
  "provider": {
    "myprovider": {
...
The `limit` fields allow Ishi to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `ishi auth list` to see if the credentials
...
2. For custom providers, check the ishi config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your ishi config.
