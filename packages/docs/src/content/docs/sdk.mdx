...
The ishi JS/TS SDK provides a type-safe client for interacting with the server.
Use it to build integrations and control ishi programmatically.
...
Create an instance of ishi:

```javascript
import { createIshi } from "@pixelml/sdk"

const { client } = await createIshi()
```
...
You can pass a configuration object to customize behavior. The instance still picks up your `ishi.json`, but you can override or add configuration inline:

```javascript
import { createIshi } from "@pixelml/sdk"

const ishi = await createIshi({
  hostname: "127.0.0.1",
  port: 4096,
  config: {
...
console.log(`Server running at ${claw.server.url}`)
...
If you already have a running instance of ishi, you can create a client instance to connect to it:

```javascript
import { createIshiClient } from "@pixelml/sdk"

const client = createIshiClient({
...
```typescript
import type { Session, Message, Part } from "@pixelml/sdk"
```
