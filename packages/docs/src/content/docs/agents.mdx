...
There are two types of agents in Ishi; primary agents and subagents.
...
Ishi comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
Ishi comes with two built-in subagents, **General** and **Explore**. We'll look at this below.

---

## Built-in

Ishi comes with two built-in primary agents and one built-in subagent.
...
Configure agents in your `ishi.json` config file:
...
- Global: `~/.config/ishi/agent/`
- Per-project: `.ishi/agent/`

```markdown title="~/.config/ishi/agent/review.md"
---
...
If no temperature is specified, Ishi uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global Ishi config and the project specific config.
...
The model ID in your Ishi config uses the format `provider/model-id`. For example, if you're using [Pixel ML OpenRouter](/docs/pixelml), you would use `ishi/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Run `ishi models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/PixelML/ishi).
:::
...
```markdown title="~/.config/ishi/agent/docs-writer.md"
---
...
```markdown title="~/.config/ishi/agent/security-auditor.md"
---
