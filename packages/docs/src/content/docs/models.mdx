...
Most popular providers are preloaded by default. If you've added the credentials for a provider through the `/connect` command, they'll be available when you start Ishi.
...
Here are several models that work well with Ishi, in no particular order. (This is not an exhaustive list):
...
Here the full ID is `provider_id/model_id`. For example, if you're using [Pixel ML OpenRouter](/docs/pixelml), you would use `pixelml/semantic-routed-model` for GPT 5.1 Codex.
...
```jsonc title="ishi.jsonc" {7-12,19-24}
{
  "$schema": "https://ishi.so/config.json",
  "provider": {
    "openai": {
...
When Ishi starts up, it checks for models in the following priority order:

1. The `--model` or `-m` command line flag. The format is the same as in the config file: `provider_id/model_id`.

2. The model list in the Ishi config.
